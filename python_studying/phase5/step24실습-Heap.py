...
class EmergencyRoom:
    ...
    def register_patient(self, name, urgency_level):
        ...
        self.emergency_heap.insert((urgency_level, self.patient_num, name))
        self.patient_num += 1
        print(f"{name}님 접수되었습니다.")
    ...
    def call_next_patient(self):
        """다음 환자 호출"""
        if self.emergency_heap.is_empty():
            print("대기 환자 없음")
            return
        _ , num, name = self.emergency_heap.extract_min()
        print(f"{num}번 {name}님 진료받으러 오세요.")
    ...
    def show_waiting_patients(self):
        """대기 중인 환자 확인"""
        print(f"대기 중인 환자 수: {self.emergency_heap.size()}")
        if self.emergency_heap.is_empty():
            print("다음 진료 대상: 없음")
        else:
            _, num, name = self.emergency_heap.peek()
        print(f"다음 진료 대상: 접수번호 {num}번 {name}님")
        # TODO: 대기 중인 환자 수 출력
...
def find_top_k_scores(scores, k):
    ...
    score_heap = MinHeap()
    for score in scores:
        score_heap.insert(-score)
    
    result = []
    for i in range(k):
        temp  = score_heap.extract_min()
        result.append(-temp)
...
class MedianTracker:
    ...
    def add_number(self, nums):
        if self.max_heap.is_empty():
            self.max_heap.insert(-nums)
            return
        
        max_root = self.max_heap.peek() * (-1)

        if nums <= max_root:
            self.max_heap.insert(-nums)
        else:
            self.min_heap.insert(nums)

        if self.max_heap.size() - self.min_heap.size() > 1:
            # max_heap이 너무 큼
            moved = self.max_heap.extract_min() * (-1)
            self.min_heap.insert(moved)
        elif self.min_heap.size() - self.max_heap.size() > 1:
            # min_heap이 너무 큼  
            moved = self.min_heap.extract_min()
            self.max_heap.insert(-moved)
...
for num in numbers:
    ...
    print(f"숫자 {num} 추가 → 중간값: {tracker.get_median():.2f}")
